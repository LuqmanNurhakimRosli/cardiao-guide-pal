...
// 1. Move Assessment components
const assessmentComponents = [
  'AfConfirmationModal.tsx',
  'AfEvidenceCard.tsx',
  'Cha2ds2VascHybrid.tsx',
  'ClinicGateBanner.tsx',
  'HasBledCalculator.tsx',
  'MissingDataCard.tsx',
  'ScoreEvidenceModal.tsx'
];

assessmentComponents.forEach(f => {
  moveFile(`src/components/cdss/${f}`, `src/pages/assessment/components/${f}`);
});

// 2. Move usePatientState hook
moveFile('src/cdss/usePatientState.ts', 'src/pages/assessment/hooks/usePatientState.ts');
try { fs.rmdirSync('src/cdss'); } catch (e) {}

// 3. Move API & Service
moveFile('src/api/cdssApi.ts', 'src/pages/assessment/api/cdssApi.ts');
try { fs.rmdirSync('src/api'); } catch (e) {}

moveFile('src/services/cdssService.ts', 'src/pages/assessment/api/cdssService.ts');
try { fs.rmdirSync('src/services'); } catch (e) {}

console.log('Assessment files moved.');

// 4. Create src/pages/assessment/index.tsx
// It contains PatientDashboard and helper sub-components from src/routes/index.tsx
const assessmentPageContent = `import { Link } from "@tanstack/react-router";
...
ensureDir('src/pages/assessment');
fs.writeFileSync('src/pages/assessment/index.tsx', assessmentPageContent, 'utf8');
console.log('Created: src/pages/assessment/index.tsx');

// 5. Update src/routes/index.tsx to thin route
const routeIndexContent = `import { createFileRoute, redirect } from "@tanstack/react-router";
import { z } from "zod";
...
fs.writeFileSync('src/routes/index.tsx', routeIndexContent, 'utf8');
...
console.log('Step 2 migration finished.');
