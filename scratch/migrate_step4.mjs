...
// 1. Create src/pages/summary/index.tsx
const summaryPageContent = `import { Link } from "@tanstack/react-router";
...
ensureDir('src/pages/summary');
fs.writeFileSync('src/pages/summary/index.tsx', summaryPageContent, 'utf8');
console.log('Created: src/pages/summary/index.tsx');

// 2. Update src/routes/summary.tsx
const routeSummaryContent = `import { createFileRoute } from "@tanstack/react-router";
import { z } from "zod";
import {
  listPatients,
  getPatientWithCdss,
...
export const Route = createFileRoute("/summary")({
  ...
  component: SummaryRouteComponent,
});

function SummaryRouteComponent() {
  const { current, actions } = Route.useLoaderData();
  return <SummaryPage current={current} actions={actions} />;
}
`;

fs.writeFileSync('src/routes/summary.tsx', routeSummaryContent, 'utf8');
console.log('Updated: src/routes/summary.tsx');
