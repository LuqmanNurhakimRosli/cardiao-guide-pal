      ...
      if (criteria.length >= 2) {
        ...
        apixAlerts.push(
          buildAlert({
            ...
            category: "drug-dose",
            group: "Drug Safety",
            title: "Review Apixaban dose (CrCl 15–29 mL/min)",
            detail: `CrCl = ${ctx.clcr} mL/min (15–29 mL/min). Consider dose reduction to 2.5 mg BD.`,
            ...
            recommendation: "Consider reducing Apixaban to 2.5 mg BD and monitor renal function closely.",
            supporting_values: { clcr: ctx.clcr },
            priority: CLINICAL_RULES.precedence.doseAdjustment,
            action: {
              kind: "medication",
      ...
      if (reasons.length > 0) {
        edoxAlerts.push(
          buildAlert({
            ...
            category: "drug-dose",
            group: "Drug Safety",
            title: "Reduce Edoxaban dose to 30 mg OD",
            detail: `Criteria met: ${reasons.join("; ")}.`,
            rationale: reasons,
            guideline: "Edoxaban Product Monograph — Dose Adjustment Criteria",
            recommendation: "Reduce Edoxaban from 60 mg OD to 30 mg OD.",
    ...
    if (ctx.clcr != null && ctx.clcr < CLINICAL_RULES.rivaroxaban.clcrAvoid) {
      ...
      rivaAlerts.push(
        buildAlert({
          ...
          category: "drug-dose",
          group: "Drug Safety",
          title: "Reduce Rivaroxaban dose to 15 mg OD",
          ...
          recommendation: "Reduce Rivaroxaban from 20 mg OD to 15 mg OD.",
          supporting_values: { clcr: ctx.clcr },
          priority: CLINICAL_RULES.precedence.doseAdjustment,
          action: {
            kind: "medication",
      ...
      if (ctx.clcr != null && ctx.clcr >= 30 && ctx.clcr <= CLINICAL_RULES.dabigatran.clcrReduce) {
        dabiAlerts.push(
          buildAlert({
            ...
            category: "drug-dose",
            group: "Drug Safety",
            title: "Reduce Dabigatran dose to 110 mg BD (CrCl 30–50 mL/min)",
            ...
            recommendation: "Reduce Dabigatran from 150 mg BD to 110 mg BD.",
            supporting_values: { clcr: ctx.clcr },
            priority: CLINICAL_RULES.precedence.doseAdjustment,
            action: {
              kind: "medication",
      ...
      if (onMed("verapamil")) {
        dabiAlerts.push(
          buildAlert({
            ...
            recommendation: "Reduce Dabigatran to 110 mg BD while on Verapamil.",
            priority: CLINICAL_RULES.precedence.doseAdjustment,
            action: {
              kind: "medication",
              medication: "Dabigatran",
