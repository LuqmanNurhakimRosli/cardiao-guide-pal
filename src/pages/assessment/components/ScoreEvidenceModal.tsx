...
import { ShieldAlert, AlertTriangle, CheckCircle2, ChevronRight, Activity, Heart, Info, PencilLine } from "lucide-react";
...
export function Cha2ds2VaConfirmationModal({
  ...
  const items = [
    { label: "CHF / LV dysfunction", active: draft.chf ?? c.chf, source: draft.chf !== undefined ? "Clinician" : "EMR" },
    { label: "Hypertension", active: draft.hypertension ?? c.hypertension, source: draft.hypertension !== undefined ? "Clinician" : "EMR" },
    { label: `Age ${age} (≥75: +2, 65–74: +1)`, active: age >= 65, points: age >= 75 ? "+2" : age >= 65 ? "+1" : "0", source: draft.age !== undefined ? "Clinician" : "EMR" },
    { label: "Diabetes Mellitus", active: draft.diabetes ?? c.diabetes, source: draft.diabetes !== undefined ? "Clinician" : "EMR" },
    { label: "Stroke / TIA History", active: draft.stroke ?? c.stroke, points: "+2", source: draft.stroke !== undefined ? "Clinician" : "EMR" },
    { label: "Vascular Disease", active: draft.vascular ?? c.vascular, source: draft.vascular !== undefined ? "Clinician" : "EMR" },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-lg border border-border bg-card p-5 shadow-2xl animate-in fade-in zoom-in-95">
        ...
        <p className="mb-3 text-xs text-muted-foreground leading-relaxed">
          The CDSS analyzed stroke risk factors from patient comorbidities and demographics. Please confirm the clinical profile before continuing.
        </p>
          ...
          {items.map((it, idx) => (
            <div
              ...
              className={`flex items-center justify-between rounded px-2 py-1 ${
                it.active ? "bg-primary/5 border border-primary/20 font-medium" : "text-muted-foreground"
              }`}
            >
              <div className="flex items-center gap-2">
...
export function HasBledConfirmationModal({
  ...
  const items = [
    { label: `Hypertension (Latest BP: ${bp})`, active: draft.hb_hypertension ?? (patient.vitals?.bp_latest ? Number(bp.split("/")[0]) > 160 : false), source: "Vitals/EMR" },
    { label: `Abnormal Renal (${creatinine ? `${creatinine} µmol/L` : "Normal"})`, active: draft.hb_abnormalRenal ?? ((creatinine ?? 0) >= 200), source: "Labs" },
    { label: "Abnormal Liver Function", active: draft.abnormalLiver ?? false, source: "Clinician" },
    { label: "Prior Stroke History", active: draft.hb_stroke ?? (patient.comorbidities?.stroke ?? false), source: "EMR" },
    { label: "Prior Major Bleeding History", active: draft.bleedingHistory ?? false, source: "Clinician" },
    { label: "Labile INR (TTR <60%)", active: draft.hb_labileINR ?? false, source: "Labs/Clinician" },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-lg border border-border bg-card p-5 shadow-2xl animate-in fade-in zoom-in-95">
        ...
        <p className="mb-3 text-xs text-muted-foreground leading-relaxed">
          The CDSS pre-filled bleeding risk factors from vitals, labs, and medications. Please verify reversible risk factors.
        </p>
        ...
        <div className="mb-3 max-h-48 overflow-y-auto rounded-md border border-border bg-background/70 p-2.5 space-y-1.5 text-xs">
          {items.map((it, idx) => (
            <div
              key={idx}
              className={`flex items-center justify-between rounded px-2 py-1 ${
                it.active ? "bg-amber-500/10 border border-amber-500/20 font-medium" : "text-muted-foreground"
              }`}
            >
              <div className="flex items-center gap-2">
                <span className="font-mono text-[10px] uppercase text-muted-foreground/80 bg-muted px-1 rounded">
              ...
              <span className={it.active ? "text-amber-600 dark:text-amber-400 font-semibold" : "text-muted-foreground"}>
        ...
        <div className="mb-3 rounded border border-border bg-muted/40 p-2.5 text-xs text-foreground">
          ...
          <p className="mt-0.5 text-muted-foreground text-[11px] leading-relaxed">
            A high HAS-BLED score is <strong>not a contraindication</strong> to anticoagulation. It indicates the need to identify and correct modifiable bleeding risk factors and schedule closer clinical monitoring.
          </p>
        </div>
